...
- **CGI-050 Lena Fischer** — a joiner with a start date of 15 September 2026. Her account has **not yet been created**: CGI-POL-003 section 4.3.3 forbids creating it before 11 September 2026 (two business days before the start date), and section 4.3.5 keeps it **inactive** until her policy sign-off and MFA enrolment are complete. Nothing is wrong here; this is the control working as designed.
...
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| CGI-014 | Priya Raman | Engineering | 2023-02-06 | Signed | 2026-09-02 | 1.0 | Verified | Hardware Key | Yes | 2026-09-02 | 2027-09-02 | Jerry Olugboye | 2026-09-02 |
...
| CGI-039 | Amara Boateng | Customer Success | 2024-11-04 | Signed | 2026-09-04 | 1.0 | Verified | Authenticator App | Yes | 2026-09-04 | 2027-09-04 | Jerry Olugboye | 2026-09-04 |
...
1. Copy the **Main tracker** table and paste it into Google Sheets or Excel. Use **Paste special → Values only** so the pipe characters do not carry across as formatting.
2. If it lands in a single column, select the column and use **Data → Split text to columns**, choosing the pipe character `|` as the separator, then delete the empty first and last columns and the separator row of dashes.
3. Freeze the header row: **View → Freeze → 1 row**.
4. Add data validation to the three status columns using the permitted values above: **Data → Data validation → Dropdown from a list**. This is what stops the tracker degrading into free text within a month.
5. Add conditional formatting: green for `Signed` / `Verified` / `Yes`, amber for `Pending`, red for `Not Started` / `Not Enrolled` / `No`.
6. Build the monthly summary with `COUNTIF`. For the AUP sign-off rate, with data in rows 2 to 51:
   `=COUNTIF(D2:D51,"Signed")/COUNTA(D2:D51)`
   then format the cell as a percentage.
7. Protect the sheet so only People and Operations can edit, and share it read-only with the CTO.
